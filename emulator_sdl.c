...
void updateController()
{
    ...
    float cx = (float) SDL_JoystickGetAxis(stick, 0) / 32767.0f;
    float cy = (float) SDL_JoystickGetAxis(stick, 1) / 32767.0f;
    ...
    if ( (SDL_JoystickGetButton(stick, 8) ==1) && (SDL_JoystickGetButton(stick, 9)==1) )
    {
    ...
    if ( SDL_JoystickGetButton(stick, 8) ) // select
    {
    ...
    if ( SDL_JoystickGetButton(stick, 9) ) // START
    {
...
int main(int argc, char *argv[])
{
    ...
    if (!setupKeyboard())
    {
        fprintf(stderr, "Configuring SDL joystick failed.\n");
        return 1;
    }
